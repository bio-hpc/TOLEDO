...
echo $FRAMES
N=$(($FRAMES/5)) #Number 5 can be changed
echo $N

if [[ "$NL" -ge "2" ]];then
for i in $(seq 1 1 $NL)
do
sbatch $G --mem=20G --time=24:00:00 --cpus-per-task=24 -J TOLEDO -p $p --output=TOLEDO.out --output=TOLEDO.err -n 1 ../../Toledo_AN_M.sh -l $i -f $N -t $time -r $ruta -p $p -g $G
done
else 
sbatch $G --mem=20G --time=24:00:00 --cpus-per-task=24 -J TOLEDO -p $p --output=TOLEDO.out --output=TOLEDO.err -n 1 ../../Toledo_AN_1.sh -f $N -t $time -r $ruta -p $p -g $G
fi
